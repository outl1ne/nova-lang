<template>
  <div
    ...
    <h3 class="flex items-center font-normal text-white mb-6 text-base no-underline" v-on:click="toggleClass()">
      <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="#b3c1d1"
           class="sidebar-icon">
      ...
      <span class="sidebar-label dim items-center">
        {{locales[activeLocale]}}
    </span>
    </h3>
    <div>
      <ul class="list-reset mb-8">
        <li class="leading-wide mb-4 text-sm ml-8 no-underline dim"
            v-for="(value, key) in locales"
            v-bind:class="{'router-link-exact-active router-link-active': locales[activeLocale]===value}"
            v-on:click="changeLocale(key)">
          {{value}}
        </li>
      </ul>
    </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            setLocale() {
                Nova.request()
                    .get(`/nova-vendor/nova-lang/novaLangGetActiveLocale`)
                    .then(
                        response => {
                            this.activeLocale = response.data;
                        },
                        () => {
