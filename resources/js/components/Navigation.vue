...
export default {
  ...
  methods: {
    ...
    updateActiveLocaleFromStore(newLocale) {
      Nova.request()
        .post(`/nova-vendor/nova-lang/novaLangUpdateActiveLocale/${newLocale}`)
        .then(
          () => {
            this.isActive = false;
          },
          () => {
            this.$toasted.show('Failed to update locale!', { type: 'error' });
          }
        );
    },
